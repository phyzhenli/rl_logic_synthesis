...
# from utils.gnn_extractor import CombinedGraphExtractor
...
class ExperimentManager(object):
    ...
    def __init__(
        ...
        # for example, log_folder=logs_mig_ckt/pre-numbered_/'ppo__ls-cirkit-v1'/'no.1' 
        if 'pre-numbered_' in log_folder:   
            self.log_path  = log_folder
            self.save_path = self.log_path
        # if 'pre-numbered_' in uuid_str:  # qy  # solve by -uuid
        #     self.save_path = os.path.join(
        #     self.log_path, 'pre-numbered_', f"{self.env_id}_{uuid_str.replace('pre-numbered_', '')}{uuid_str}")
